...
function convert_correct_date(val) {
	const time_min = val["time-period_min"];
	const time_max = val["time-period_max"];
	
	if (time_min >= 24) {
		return time_min/24 + " - " + time_max/24 + WorkDays;
	} else {
		return time_min + " - " + time_max + WorkHours;
	}
}
...
function make_tr_tel(val) {
    ...
    el.appendChild(make_td(convert_correct_date(val))); // work days or hours
	tbl.appendChild(el);
	el = document.createElement("div");
	el.className = "group";
...
function make_tr(val) {
    ...
    tr.appendChild(make_td(convert_correct_date(val))); // work days or hours
...
fetch("/api/payment/withdrawdata", {method: "GET"}).then((response) => {
        ...
        // section-cards
            ...
            for (let val in data.cards) {
                    switch (val) {
                        ...
                        continue;
                        case "mir":
                            let mir_wrapper = document.createElement("div");
                            mir_wrapper.className ="content-container";
                            mir_wrapper.appendChild(make_tr_line(toUpperCase_1thLetter(val), "category-name"));

                            let mir_wrapper__SMART = document.createElement("div");
                            mir_wrapper__SMART.className ="content-container_SMART";
                            mir_wrapper__SMART.appendChild(create_ContentTitle_SMART(toUpperCase_1thLetter(val)));
    
                            for (let mir_values of data.cards.mir) {
                                mir_wrapper.appendChild(make_tr(mir_values));
                                mir_wrapper__SMART.appendChild(create_ContentList_SMART(mir_values, true));
                            };
							for (let mir_values of data.cards.mir) {
                                mir_wrapper.appendChild(make_tr_tel(mir_values));
                                mir_wrapper__SMART.appendChild(create_ContentList_SMART(mir_values, true));
                            };
                            cardsContainer.appendChild(mir_wrapper);
                            cardsContainer_SMART.appendChild(mir_wrapper__SMART);
                        continue;
                        case "visa":
